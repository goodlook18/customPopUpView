...
- (instancetype)initWithTitle:(NSString *)title icon:(UIImage *)icon message:(NSString *)message delegate:(id<DPPopUpViewDelegate>)delegate buttonTitles:(NSString *)buttonTitles, ... {
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        _icon = icon;
        _title = title;
        _message = message;
...
- (void)show {
    ...
    if(windowViews && [windowViews count] > 0){
        ...
        //直接在window上加，不在subview上加，因有时subview不是想要被加载的视图
